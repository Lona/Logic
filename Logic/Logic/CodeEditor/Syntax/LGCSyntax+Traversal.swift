...
extension LGCSyntaxNode {
    ...
    private func reduceChildren<Result>(
        ...
        f: @escaping (Result, LGCSyntaxNode, LGCSyntaxNode.TraversalConfig) throws -> Result
    ) rethrows -> Result {
        switch self {
    ...
    public func reduce<Result>(
        initialResult: Result,
        f: @escaping (Result, LGCSyntaxNode,  LGCSyntaxNode.TraversalConfig) -> Result
        ) -> Result {
        return reduce(config: TraversalConfig(), initialResult: initialResult, f: f)
    }

    public func forEachDescendant(
        config:  LGCSyntaxNode.TraversalConfig,
        f: @escaping (LGCSyntaxNode,  LGCSyntaxNode.TraversalConfig) throws -> Void
    ) rethrows {
